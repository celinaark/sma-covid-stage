...
// Definition des variables atatiques
      int              World::_size;
      int              World::_nbIteration;
      ...
      vector<int>      World::_tailleVilles;
      vector<int>      World::_nbHabitants;
      vector<int>      World::_coordonneesX;
      vector<int>      World::_coordonneesY;
      

   
      // On considere ici le taux de mortalite reel, en supposant qu'on a deux fois plus de chance d'avoir �t� en r�animation (50% des gens en r�a meurent) et deux fois plus de chance d'avoir �t� hospitalis� (50% de personnes hospitalis�s vont en r�a)
      ...
      deque<int>        World::_timelineVaccination;
      deque<int>        World::_timelineVaccinationRappel;
      deque<int>        World::_timelineMasqueTissu;
      deque<int>        World::_timelineMasqueChir;
      deque<int>        World::_timelineMasqueFFP2;
      deque<int>        World::_timelineGelHydroalcolique;
      deque<int>        World::_timelineConfinement;
      deque<int>        World::_timelineCouvreFeu;
      deque<int>        World::_timelineMedicament;
      deque<int>        World::_timelineDeplacementLimites;
             ...
             bool               inLog)
{
   ...
   _tabAugmentionContaminationParLieu['H'] = _tauxAugmentationContaminationHopitaux;
   _tabAugmentionContaminationParLieu['G'] = _tauxAugmentationContaminationGym;
   _tabAugmentionContaminationParLieu['C'] = _tauxAugmentationContaminationClub;
   _tabAugmentionContaminationParLieu['S'] = _tauxAugmentationContaminationShop;
   _tabAugmentionContaminationParLieu['R'] = _tauxAugmentationContaminationRestaurant;


   for(int i=0; i<_tailleVilles.size();i++)
   {
      
      _listVille.push_back(new City(_tailleVilles[i],_nbHabitants[i],_coordonneesX[i],_coordonneesY[i]));
   }
   ...
   */
   ...
   if(_carte[inRow][inColumn]->getState() == 3)
   {
         ...
         if(randValue < 1 - _timelineAsymptomatique[_iteration])
         {
            ...
            _carte[inRow][inColumn]->toConfined();
...
{  
   ...
   std::vector<float> tableTauxHospitalisationByAge = 
                     inSimulationParams->getTableTauxHospitalisationByAge();
                     
   std::vector<int>   tableTailleHopitaux = 
   ...
   std::vector<float> histogrammeContamination = 
   ...
   std::vector<float> defaultVariantHistoConta =
                     inSimulationParams->getDefaultVariantHistoConta();
   
   std::vector<float> variantsHistoConta =
                     inSimulationParams->getVariantsHistoConta();
               
   std::vector<int> dureeVariants =
                     inSimulationParams->getDureeVariants();
                     
   std::vector<float>  pourcentAsymptomatiqueVariants =
                     inSimulationParams->getPourcentAsymptomatiqueVariants();
                     
   std::vector<float> tableTauxHospitalisationByAgeVariants =
   ...
   _tailleVilles = tailleVilles;
   _nbHabitants = nbHabitants;
